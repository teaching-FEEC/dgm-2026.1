...
def mse_psd(real_emg: torch.Tensor, pred_emg: torch.Tensor, intervals: list):
    ...
    for key in list(intervals):
        ...
        for channel in range(8): # loop over each channel
            ...
            psd_x_normalized = (psd_x[int(key[0]/new_res):int(key[1]/new_res)] - psd_x.mean())/(psd_x.std() + 10**(-8))
            psd_y_normalized = (psd_y[int(key[0]/new_res):int(key[1]/new_res)] - psd_y.mean())/(psd_y.std() + 10**(-8))
