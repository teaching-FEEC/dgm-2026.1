# Adapted from FakeVLM/collators/llava_1_5.py and
# FakeVLM/collators/chat_template_monkey_patch.py.
#
# Three modifications vs the original:
#   1. Collects raw PIL images and runs freq_extractor.preprocess()
...
class ExtendedLlavaCollator:
    """Data collator that produces both CLIP pixel_values and freq_pixel_values.

    Reimplements the LLaVA 1.5 collator from FakeVLM with frequency-domain
    support.  The image token expansion count is increased by num_freq_tokens
    so that the model's masked_scatter merge places 577 features (576 CLIP +
    1 frequency) into the sequence.
    """
    ...
    def __call__(
        ...
        # Monkey-patch tokenizer to include BOS tokens
        ...
        # --- Modification 1: collect raw images for frequency preprocessing ---
        ...
        if len(all_raw_images) > 0:
            freq_pixel_values = self.freq_extractor.preprocess(all_raw_images)
        else:
        ...
        for system_prompt, cur_images, cur_convs in zip(
            ...
            if temp_vision_inputs.get("pixel_values") is not None:
                if (
                    ...
                    # --- Modification 2: add frequency tokens ---
                    num_image_tokens += self.num_freq_tokens
        ...
        # --- Modification 3: return freq_pixel_values ---
        return dict(
            **vision_inputs,
            input_ids=input_ids,
            labels=labels,
